...
-- Just to check another FiestaWare instance is running and close it
local uid = tick() .. math.random(1,100000) .. math.random(1,100000)
if shared.fw1 and shared.fw1.close and shared.fw1.uid~=uid then shared.fw1:close() end

-- Main shitty script should start below here

warn("FW1_MAIN : Running script...")
...
do
    ...
    spawn(function()
        while fw1 and fw1.enabled do -- Some games are gay
...
do
    ...
    settings.fileName = "FiestaWare_V1_settings.txt" -- Lovely
    settings.saved = {}
...
do
    ...
    setmetatable(aimbot, {
        __index = function(self, index)
            ...
            warn(("fw1_ERROR : AimbotSettings : Tried to index %s"):format(tostring(index)))
        end;
...
do
    ...
    setmetatable(crosshair, { -- yes I know it is easier ways to add this but that requires effort
        __index = function(self, index)
            ...
            warn(("fw1_ERROR : CrosshairSettings : Tried to index %s"):format(tostring(index)))
        end;
        __newindex = function(self, index, value)
            if typeof(value) ~= "function" then
...
do
    ...
    setmetatable(tracers, {
        __index = function(self, index)
            ...
            warn(("fw1_ERROR : TracersSettings : Tried to index %s"):format(tostring(index)))
        end;
        __newindex = function(self, index, value)
            if typeof(value) ~= "function" then
...
do
    ...
    setmetatable(esp, {
        __index = function(self, index)
            ...
            warn(("fw1_ERROR : EspSettings : Tried to index %s"):format(tostring(index)))
        end;
        __newindex = function(self, index, value)
            if typeof(value) ~= "function" then
...
do
    ...
    setmetatable(boxes, {
        __index = function(self, index)
            ...
            warn(("fw1_ERROR : BoxesSettings : Tried to index %s"):format(tostring(index)))
        end;
        __newindex = function(self, index, value)
            if typeof(value) ~= "function" then
...
do
    ...
    function visuals.step()
        ...
        if credits == nil then
            credits = newdrawing("Text", {
                Text = "FiestaWare"; -- yes now be happy this is free
    ...
    spawn(function()
        while fw1 and fw1.enabled do -- I dont know why I am doing this
            for i,v in pairs(hashes) do
                hashes[i] = nil
                wait()
            end
...
do
        ...
        for i,v in pairs(engine) do
            ...
            xpcall(v.func, function(err)
                if (DEBUG_MODE == true) then
                    Warn("fw1_ERROR (RENDERSTEPPED) : Failed to run %s! %s | %s", v.name, tostring(err), debug.traceback())
                end
                engine[i] = nil
            end, run.dt)
            profileend(v.name)
    ...
    bindEvent(heartbeat, function(delta)
        ...
        for i,v in pairs(heartengine) do
            ...
            xpcall(v.func, function(err)
                if (DEBUG_MODE == true) then
                    Warn("fw1_ERROR (HEARTBEAT) : Failed to run %s! %s | %s", v.name, tostring(err), debug.traceback())
                end
                heartengine[i] = nil
            end, delta)
            profileend(v.name)
    ...
    bindEvent(stepped, function(delta)
        ...
        for i,v in pairs(whilerender) do
            ...
            xpcall(v.func, function(err)
                if (DEBUG_MODE == true) then
                    Warn("fw1_ERROR (STEPPED) : Failed to run %s! %s | %s", v.name, tostring(err), debug.traceback())
                end
                heartengine[i] = nil
            end, delta)
            profileend(v.name)
...
do
    ...
    setmetatable(fw1, { -- ok safazi be happy now
        __newindex = function(self, index, value)
...
warn("fw1_MAIN : Reached end of script")
